...
### [2026-06-10] ボドゲ会ウェブサイト — デザイン全面リニューアル（本格仕上げ）

**作業内容：**

- **「無料AI製に見えない」本格デザインへ全面改装**（`projects/rough/index.html` を書き直し）
  - フォント刷新：見出し＝明朝体（Shippori Mincho B1）／英字ラベル＝Cinzel／本文＝Zen Kaku Gothic New（すべてGoogle Fonts・**無料**。読み込めない環境では system-ui に自動フォールバック）
  - 絵文字アイコン（🃏🙋📖🍹等）→ 線画SVGアイコンに全置換
  - 全面に微細ノイズ質感・ヒーローに格子＋金グロー2灯で奥行きを追加
  - スクロール連動フェードイン（IntersectionObserver・prefers-reduced-motion対応）
  - CTAボタンに光沢アニメーション・FAQアコーディオンに開閉アニメーション
  - 最終CTAセクション（「次回は6月21日、名駅で。」）を新設
  - 写真ギャラリーにホバーズーム・グラデーションオーバーレイ
- **バグ修正**：CSSの `#games` 重複定義ブロックと余分な `}` を解消
- **検証**：CSS波括弧・HTMLタグ開閉・JS構文チェックすべてOK（環境制限でブラウザスクリーンショットは不可）

**成果物：**
- `projects/rough/index.html`（全面書き直し。掲載データ＝ゲーム36本・FAQ・チャット応答・開催情報・URLはすべて温存）
- ブランチ `claude/rough-recruitment-page-mcv286` にプッシュ（本番反映はメインブランチへのマージ後）

**次のアクション：**
- 実機（スマホ・iPad縦持ち）での表示確認
- 写真のモザイク処理版への差し替え（従来からの持ち越し）
- ボドゲゴーURLのイベントごと更新（HTML内に「★更新ポイント」コメントあり）

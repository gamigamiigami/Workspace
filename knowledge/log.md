...
### [2026-06-16] 席替えツール — 年度・クラス管理、A4印刷/PDF対応、教室の編集・削除機能を追加

**作業内容（伊神さん要望2点）：**

- **年度＋クラス名でのデータ管理**：教室に `year`（例：R8）と `className`（例：1-1）フィールドを追加。ヘッダーの「＋年度・クラスを開く/作る」で年度・クラス名を入力 → 同じ組み合わせが既存なら新規作成せずそのデータ（生徒・履歴含む）を開く、なければ新規作成。これにより来年度以降も「R8 1-1」のように選べば前回までの隣席履歴・同班履歴がそのまま使える
- **教室の編集・削除**：ヘッダーに「✏️編集」を追加。年度・クラス名の修正（重複チェックあり）と、クラスデータ全体の削除（生徒・履歴含む、クラスが2つ以上あるときのみ）に対応。これまで作成後に教室名を直せなかった/削除できなかった抜けを解消
- **A4印刷・PDF対応**：`@page { size: A4; margin: 12mm }` を追加。名簿タブに印刷用ヘッダー（クラス名＋日付）を追加し、座席表は名簿の次ページから印刷されるよう `page-break-before` を設定。班カードは `page-break-inside: avoid` でページ途中で割れないように。印刷ボタンの説明文に「PDFに保存」の案内を追加（ブラウザの印刷画面からPDF保存できる旨）
- **旧データ移行**：`migrateState` で旧形式の教室名（年度を含まない自由テキスト）から `year`/`className` を推測して補完
- **検証**：Node.js + 自作DOMスタブで `init()` を含む全体スクリプトを実行し、年度・クラス名での新規作成/既存データの呼び出し（重複防止・履歴の継続を確認）、リネーム時の重複チェック、削除、名簿タブの印刷ヘッダー描画までEnd-to-Endで確認

**次のアクション：**
- iPad実機テスト待ち（年度・クラス選択の操作感、印刷ダイアログからのPDF保存、A4レイアウトの見た目）

---

### [2026-06-12] 席替えツール — 男女色分け・席ベースのグループ・空席設定を追加

**作業内容（伊神さん要望3点）：**

- **男女登録＋席の色分け**：生徒に性別（男/女/なし）を追加。一覧の「男/女」ボタンでワンタップ切替、まとめて追加は「名前,男」形式に対応。座席表では男子＝うすい青、女子＝うすいピンクの背景色
- **グループを「席ベース」に全面変更**：班から「この色をぬる」を選んで席をタップ→席に班色がつく方式（誰がその班になるかは席替えの結果で決まる）。班＝わくの色にして男女の背景色と両立。名簿・同班履歴チェック・履歴スナップショットも席ベースに書き換え。班名はA班・B班…と自動命名
- **空席設定**：教室タブに「🪑空席にする・もどす」モードを追加（机はあるが誰も座らない席。通路とは別）。配置対象から除外
- **旧データ移行**：memberIds形式の班→席の班へ自動変換（migrateState、ひかえファイル読込時も適用）
- **検証**：Nodeスタブテストで空席除外（26/26席）・固定席・NG回避100回維持、席ベースgroupSnapshot 26件、旧形式移行、コの字プリセット回帰をすべて確認

**次のアクション：**
- iPad実機テスト待ち（男女の色の見やすさ・班の色ぬり操作・空席の挙動）

---

### [2026-06-12] 席替えツール — 初心者向けUX改善＋机の向きの視覚化（コの字対応）

**作業内容（伊神さん要望2点）：**

- **迷わない導線**：タブを「①教室→②生徒→③席替え」の番号順に再編成。①教室タブ冒頭に「つかい方は3ステップ」ガイドカード、各タブ末尾に「つぎへ→」ボタン、③席替えタブに生徒0名/席不足時の案内パネルを追加
- **教室設定の操作を簡略化**：「机を選択→ボタン」方式を廃止し、モード2つ（🔄向きをかえる／🚧通路にする）＋机タップだけの操作に変更。タップごとに 前→右→左 と向きが回る
- **隊形プリセット**：「⬜ぜんぶ前向き」「🔄コの字にする」ワンタップボタンを新設（コの字＝左列右向き・右列左向き・最後列前向き・中央通路）
- **机の向きの視覚化**：矢印に加えて机の四角自体が変形（前向き=横長＋上辺太/横向き=縦長＋左右辺太）。コの字隊形が座席表で一目でわかる
- **文言の平易化**：「CSVインポート」→「まとめていれたいとき（1行に1人ずつ）」、「JSONバックアップ」→「💾ひかえを保存」等
- **検証**：Nodeスタブテストでアルゴリズム全項目再合格＋コの字プリセットの向き/通路/席数（5×6→14席）を機械検証

**次のアクション：**
- iPad実機テスト待ち（コの字の見え方・タップで向きが回る操作感・3ステップ導線のわかりやすさ）

---

### [2026-06-12] 席替えツール — 新規開発（設計図ベース・教員向け校務ツール）

**作業内容：**

- **新規プロジェクト `projects/seat-assignment/`**（伊神さん提供の設計図どおりに実装）
  - 単一HTML・完全オフライン・localStorage永続化（`seat_tool_v1`）＋JSONバックアップ
  - 7タブ構成：🏫教室設定／👥生徒／🎨グループ／🚫NG設定／🎲席替え／📃名簿／🕐履歴
- **席替えアルゴリズム**：固定席先行配置 → Fisher-Yatesシャッフル＋貪欲配置のリスタート方式（最大300回）→ 失敗時はNG隣接を緩和して強制配置＋❌エラー表示
- **ソフト制約チェック**：前回と同じ席⚠️／過去に隣だったペアℹ️／過去に同じ班だったペアℹ️（履歴の groupSnapshot 照合）。警告タップで該当席ハイライト
- **iPad対応**：44pxタップ領域・「選択→タップ」2タップ方式・ネイティブselect・system-uiフォント
- **印刷**：`@media print` で名簿タブ（班別名簿＋座席表）のみ印刷
- **検証**：Node＋DOMスタブで機械テスト実施。100回実行で固定席・着席NG・隣席NG・無効席除外がすべて維持、同じ席警告28/28件検出、席不足エラー・緩和フォールバックも確認済み

**成果物：**
- `projects/seat-assignment/index.html`／`README.md`／`DESIGN.md`

**次のアクション：**
- iPad実機テスト待ち（タップ操作・縦持ち表示・Safari印刷・localStorage保持の確認）
- 公開する場合は deploy ワークフローへの追加＋運用ブランチへのマージが必要（failures.md の environment 保護ルール参照）

...
- **「▲前後が同じ」「▲斜めが同じ」の判定ロジックを変更**
  - 旧：「特定の2人が前回もその位置にいたか」（個人ベース）
  - 新：「前回も今回も同じ班の2人が、いまその位置にいるか」（班ベース）
  - 誰と誰かは問わず、同じ班の繰り返しだけを見る形に統一
  - 班が異なればカウントしない
  - 成績表の詳細リストに「〇〇と△△は前回も同じ班で、今回も前後／斜め」のように表示
- **「▲となりが同じ班」という新しい判定項目を追加**
  - 既存の××（特定の2人がとなり同士になる繰り返しを絶対回避）とは別物
  - 「同じ班の2人が、たまたまとなりの位置にもなっている」を新規検出
  - 既存ルールと重複しないため新規追加として実装
  - 成績表に表示、NG設定には影響しない（情報提示のみ）
- **テスト検証（2×2の小さい教室で3パターン）**
  - 「同じ班のままとなり／前後／斜めになる」パターン確認
  - 「班を外すとカウントされなくなる」パターン確認
  - すべてのテストが正しく動作することを確認（333件全合格）
- **コミット `8eb0d23` でプッシュ完了**

**発見：**
- 判定ロジックの「誰と誰か」という視点から「どの班か」という視点への転換は、実装の複雑性を下げたまま、要件理解が飛躍的に明確になった
...
### [2026-08-28] 席替えツール — 隣席NGの隣判定を現状維持で確定

**作業内容：**

- **隣席NGの隣判定について先生に確認 → 現状維持で確定**
  - 前セッションで「となり同士」を机2つずつの組（6列なら1&2/3&4/5&6、2&3はとなりでない）に変えたが、
    NG設定の隣席NGだけは上下左右すべてを隣とみなすまま残し、判断を仰いでいた
  - 理由：組の考え方だと6列の2&3が「となりでない」扱いになり、
    絶対に離したいNGペアが物理的に真横に並んでしまう
  - 先生から「それで完璧」と回答。**コード変更なし**（もともとその実装）
- **README.md に方針を明記**
  - 「この扱いでよいと先生に確認済み。今後も変更しない」を追記（コミット 48803ad）
  - 次セッションで同じ論点を蒸し返さないための記録
...
### [2026-08-28] 席替えツール — となり定義の座席形式対応・過去のとなり回避・生徒登録改善・エクスポート完全検証

**作業内容（ユーザー要望5点すべて実装完了）：**

- **「となり同士」の定義を座席配置に応じて分岐**
  - 6列（偶数）座席：机を2つずつの組にするため 2&3 は「となりでない」（通路の有無とは別の理由）
  - 5列（奇数）座席：左右どちらも「となり」
  - 内側向き座席：横組＋たての上下をとなりに含める
  - 前向き座席：横だけを「となり」。たては「前後」として分離
  - 各パターンで実装・検証完了
- **「過去に隣同士になったことがある」を新しい減点項目に追加（重み110）**
  - 15回試行での実測：ランダム17組 → 8組に削減（効果確認）
  - 席替えページの成績表に「前にもとなり ○組」として表示
- **生徒登録の名前・性別分離入力**
  - ボタン1つで入力欄を2列に分割
  - 上から順に自動組み合わせ（1行目の名前＝1行目の性別）
  - 行数ミスマッチで警告（貼りつけ中・追加後の両方）
  - 性別の受け入れ形式：`男/女/男子/女子/M/F/1/2` に対応、未登録もOK
- **性別選択の1タップ化（トグル型ボタン）**
  - 未登録から**1回で**「男」「女」のどちらでも選択可能
  - 同じボタンを再度タップすると「なし」に戻る（トグル動作）
  - 選択中のボタンは色で視覚的に区別
  - あとから一覧で変更も可能
- **エクスポート・インポート機能の完全復元を検証**
  - 実測確認：「保存 → localStorage空にする → 読みこむ」で**1文字も変わらない**復元を達成
  - 生徒・性別・履歴・NG設定・班・固定席・出席番号モード・黒板の向きまで完全に復元
  - 読みこみ後、席替えとつながりタブが正常に動作することを確認
...
**パターン・ドキュメント更新：**
- patterns.md に「機能実装前の業務要件確認」パターンを新規記録（セッション110の「よみがな機能の手戻り」から学んだ汎用パターン）
- patterns.md に「『絶対に避けたい』と『できるだけ避けたい』の分離設計」パターンを新規記録（セッション108-112での多層制約の実装ノウハウ）
- NG設定「隣席NG」は他の「となり定義」とは別に「上下左右すべて」を隣とみなす処理にしたことを記録（6列座席の特殊性への対応）

**次のアクション：**
- iPad実機テスト（新機能の操作感：ボタンタップ・入力欄分割・エクスポートの大容量出力）
- ユーザーからの反応待ち（NG設定「隣席NG」の扱いについて）

---

### [2026-08-27] 席替えツール — 避けたい優先順位・NG表示多層化・学年クラス・印刷改善

**作業内容（最終6つの機能実装）：**

- **避けたい優先順位を減点制に統一**：「となりが同じ」2000点 > 「席が同じ」600点 > 「前後が同じ」120点 > 「斜めが同じ」100点
  - 実測検証（30名30席×200回）で「となりが同じ」は200回中0組達成
  - 「席が同じ」は95%の回でゼロ（NG制約の確実性を実装で保証）
  - 「前回近くだった人と同じ班」は減点なし・お知らせのみで対応
- **NG設定のエラー表示を3層化**
  - その場でポップアップ（「🚫 男2 と 男1 がとなり同士です（隣席NG）：おしゃべり」理由メモ含む）
  - 画面上部に赤い警告（件数つき、直すと自動で消去）
  - 該当席を赤く囲んで🚫マーク表示
- **学年・クラス・出席番号を登録・切替可能に**
  - 年度／学年／クラス／表示名を登録（左上タブは表示名で表示）
  - 出席番号は「1・2・3…（通し番号）」と「男1・女1…（男女別）」で切り替え可能
  - 個別編集で手指定も可能
  - 座席表・班別名簿・全員名簿に全て反映
- **印刷時の視認性を向上**
  - 机の向き矢印・🔒固定マーク・📌予約マークを印刷から削除
  - 男女の●印・空席・通路は既に非表示のまま
  - 座席表の凡例から「（右下にも班名）」を削除
- **テスト完全合格**
  - 既存43項目 + 新機能102項目 = **145項目すべて合格**
  - 20回連続実行で全合格を確認

**パターン・UIコンポーネント：**
- patterns.md の「複数案生成→評価→採用アルゴリズム」がセッション108でも有効に活用
- NG表示3層化のUIパターンを ui-components.md へ記録検討（複数プロジェクトで応用可能）

**次のアクション：**
- iPad実機テスト（NG表示の3層・出席番号切り替え・印刷品質の最終確認）
- 本ツール v1.0 リリース準備完了

---

### [2026-08-27] 席替えツール — 最高の席替えアルゴリズム＆前回比較常時表示・操作バー貼りつき

**作業内容（ユーザー要望「確実に最高の席替えを作る」の根本的解決）：**

- **席作成アルゴリズム完全改修**：「単発の乱数生成→配置」から「最大40通り生成→前回との比較→最良案採用」へ
  - 実測データ（30名30席×20回試行）で単発生成は「前回と同じ席27人」→新アルゴリズムは「0人」（~90%削減）
  - NGペア隣接：200回試行で0件（NG制約の確実性を実装レベルで保証）
  - 複数案生成→評価→採用という「品質保証メカニズム」を確立（汎用パターンに昇華）
- **前回とのくらべを席替えページに常時表示**：別ページを見に行かなくても「✅席が同じ 0人 ✅となりが同じ 0組 ❌前後が同じ 1組」がページ上部に表示
- **座席手なおしモード新設**
  - 🔀2人を入れかえる：実行後の微調整
  - 📌今回だけ固定：実行前に「Aさんはここ」と予約（座席表に名前が見える）
- **操作バーが画面上部に貼りつく**：`position: sticky` で、スクロール不要に
  - iPad縦持ちでの「座席を見ながら操作」を実現
  - 初心者向けUX大幅向上（スクロール位置で迷わない）
- **個別設定の3種類を明確に分類**
  - 🚫 NG席・NGペア：有効期間は1年間ずっと
  - 🔒 固定席：有効期間は1年間ずっと
  - 📌 今回だけ固定：この1回だけ（記録後は自動消去）
  - 各タブの役割を明確にすることで「どの設定が何を意味するか」をユーザーが直感的に理解
- **その他利便性機能**
  - NG席に「ぜんぶNGにする・ぜんぶ解除する」：全部NGにしてから座ってよい席だけ解除という使い方に対応
  - 男女を逆にするボタン：固定席と📌は動かさず、市松模様のまま反転
  - 黒板の向き切替：名簿では「黒板が上」「黒板が下」の2枚が別ページに出るので両面印刷対応
  - 印刷時：男女の●印は出さず、空席・通路は席ごと消える

**テスト検証：**
- 既存43項目 + 新機能47項目 = **90項目すべて合格**
- 機械テストで「複数生成の候補数が有効か」「前回との差分計算が正確か」を網羅確認

**パターン昇華：**
- patterns.md に「複数案生成→評価→採用アルゴリズム」を汎用パターンとして記録
- ui-components.md に「操作バー（画面上部貼りつき）」をUIコンポーネントとして記録

**次のアクション：**
- iPad実機テスト待ち（操作バーの貼りつき具合・2人入れかえのタップ感・両面印刷の実際の出力）

---

### [2026-08-27] 席替えツール — コの字の修正・班の色の改善・男女市松模様・名簿の見直し

**作業内容（伊神さん要望6点）：**

- **コの字の形を修正**：Web検索で正しい形を確認（左右の机は教室の中を向く／いちばん後ろの机は黒板を向く／まん中を空ける／黒板側が開く）。角の机を「左右のうで」ではなく「後ろの列」あつかいにして前向きにし、コの字の横線がきれいに出るようにした。さらに**人数が入りきらないときは二重コの字（左右2列・後ろ2列）へ自動で切り替え**（6×5の一重＝14席しかなく30人学級で使えなかった問題を解消）。通路は座席表では文字を出さないようにして、隊形の形が見えるようにした
- **班の色を刷新**：連番どうしが似ないよう10色を並べ替え（赤→緑→青→オレンジ→むらさき→青緑→ピンク→茶→こんいろ→はいいろ）。連番のRGB距離163以上、全ペア97以上を機械テストで担保。班名ラベルの文字色は背景の明るさから自動で白/黒を選ぶ（オレンジに白文字で読めない問題を解消）
- **男女と班の色を両立**：背景色＝男女／太いわく＋右下の班名ラベル＝班、と役割を分離（以前は班の背景色が男女の背景色に上書きされて消えていた）。机の左上に「男」「女」の印を追加し、座席表の上に**色の凡例**を常時表示
- **男女市松模様**：チェックボックス1つで、となり合う席が男・女・男・女…とジグザグになるよう配置。席の (行+列) の偶奇で男女を振り分け、リスタートの前半2/3は厳密・後半は妥協ありで探索。完全な市松にできない場合は理由付きで通知
- **名簿の見直し**：班別名簿から席の位置（○行○列）を削除。「Aさん ○班」形式の**全員名簿**を追加（登録順・3段組みで用紙節約）

**ハマりポイント：**
- ローカルのクローンが古い状態で、リモートには自前の過去コミットが残っていた（`git fetch` 後に判明）。ローカル側の別作業（moji-kae等）が他のリモートブランチに退避済みであることを確認してから `git checkout -B` でリモートに合わせ、DESIGN.md も含めて履歴を保ったまま追記できた

**検証：** Node.js + 自作DOMスタブで43項目を機械テスト（コの字の向き/席数、二重コの字の自動切替、色差とコントラスト比、市松模様の隣接98組すべて異性、名簿に席位置が出ないこと、固定席・隣席NGの100回回帰、年度・クラス管理の回帰）すべて合格

**次のアクション：**
- iPad実機テスト待ち（コの字の見え方・市松模様のならび・班と男女の色の見分けやすさ・A4印刷/PDF保存）

...
- **ワード候補検索機能の実装**
  - ①「ことば」タブに新セクション「ワード候補をさがす」を追加
  - IPADICとwordfreqから頻度順の上位1万語をブラウザ内に内蔵（約250KB辞書データ）
  - 外部API不要・オフライン完全動作・完全無料

- **フィルタリング機能**
  - **未使用文字フィルタ**：キーワードの未使用文字のみを含む語だけを候補表示（中心機能）
  - **文字数フィルタ**：2〜8文字で検索可能
  - **交差フィルタ**：登録済みワードと交差できる語だけを候補表示

- **教育向け対応**
  - 中学生に不適切な約60語を除外リスト化（完全一致のみ、「牛乳」「殺菌」などの誤検出防止）
  - 同音異義語は最大3つまで一緒に表示（中学国語教材として活用可能）
  - ひらがな・カタカナ混在の自動判定：キーワードの表記に合わせて候補表記を統一
  - 混ざった場合は「登録ワードを全部カタカナに／ひらがなに」で一括変換可能
  - 登録した語は自動的にマイ辞書化され、★付き・最優先で表示

- **性能・動作確認**
  - 読み込み0.6秒、検索0.4秒、JSエラーなし（iPad相当で計測）
  - 検索→登録→自動生成→カギ欄まで通しで動作確認 ✅
  - 全候補が条件を満たすことを機械的に検証済み

- **ライセンス対応**
  - IPADICT・wordfreq ともに無料・再配布可
  - 画面下に出典表示・ライセンス情報表示
...
- **iPad実機確認必須**：
  ...
  - 検索の速さ
  - 出てくる語彙が授業で使えるレベルか
  - 「この語は要らない」「この単元の語が出てこない」のフィードバック待ち
- 辞書の規模（1万語）は後から増減可能

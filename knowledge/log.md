...
### [2026-07-03] my-*-maker スキル刷新 — 3スキル体制へ

**作業内容：**

- `my-tool-maker` を v2.0 に全面改訂（7フェーズ制：聞き込み→未来完成図→UI/UX→裏側設計→レビュー→実装可能宣言→Auto Mode。危険予知レビュー・購入者レビュー・技術選定ルールを整理）
- `my-lp-maker` を v2.0 に全面改訂（LP独自の「感情設計（興味→共感→期待→安心→行動）」と「コピー設計」フェーズを追加）
- `my-website-maker` を新規作成（サイトモード／ゲームモードの2モード分岐。ゲームモードは「遊びの設計」フェーズと生徒・教師視点レビューを含む）
- 3スキルにYAMLフロントマター（name/description・日本語トリガー語）を追加し自動発火しやすく
- 各スキルにワークスペース前提（iPad縦持ち・外部CDN不可・課金禁止・coding-rules等との連携）を明記
- CLAUDE.md のスキル表・ディレクトリ構成を更新

**結果：** 成功

**成果物：**
- `.claude/skills/my-website-maker/SKILL.md`（新規）
- `.claude/skills/my-lp-maker/SKILL.md`（v2.0）
- `.claude/skills/my-tool-maker/SKILL.md`（v2.0）
- `CLAUDE.md`（スキル表更新）

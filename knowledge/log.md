...
### [2026-07-04] 本セッション — X投稿ハッシュタグ戦略の見直し

**作業内容：**

- **X投稿のハッシュタグ戦略について深掘り検討**
  - 現在（2026年）のXではハッシュタグはほぼ不要であることを確認
  - ハッシュタグ経由の発見がほぼなく、むしろ「宣伝bot感」が強まる
  - 生成プロンプト「各2～3個」が逆効果の可能性を指摘

**技術的学び：**
- Xのアルゴリズム動作メカニズム（2026年版）
  - 「おすすめ欄」「タイムライン」は「本文内容 + 反応」で決定
  - ハッシュタグ検索経由の発見は市場で少ない
  - 限られた140字でハッシュタグに20～30字費やすのは文字数の無駄

**決定事項：**
- X投稿生成プロンプトを「0～1個（多くても1個、本文に自然に溶け込むもの）」に修正する方向で検討
- 「タグに頼らず本文で勝負する」という運用方針を明記する予定

**次のアクション：**
- プロンプト修正後、来週の投稿からハッシュタグ削減効果を観測

---

### [2026-07-04] 本セッション後 — X自動投稿復活・PR #137マージ完了

**作業内容：**

- **PR #137 マージ完了**
  - X自動投稿スケジュール復活（朝7:00 / 昼13:10 / 夜21:00 JST）
  - `schedule:` パラメータの復活確認
  - 「生成→自動投稿」の全自動パイプラインが復帰
...
- **シャドウバン対応撤回**
  - 実装予定だったが、実運用上の必要性がないと判定
  - 本番適用をキャンセル

**技術的学び：**
- X自動投稿のライフサイクル（一時停止→プロンプト改善→復活）が完結
- 生成品質と運用復帰を同時進行できる設計

**課題（継続）：**
- 来週のツイート質向上を実観測する必要あり
- 教員層への響き具合の確認が次の KPI

**成果物：**
- PR #137（X自動投稿復活・プロンプト強化）

**次のアクション：**
- 来週分の生成ツイートから「刺さる鉄則」効果測定
- 現場感覚での响き事例があればプロンプト再調整

...
### [汎用] 大規模ファイル読み込み時のoffset/limit による段階的処理

**用途：** JSON ログファイル、トランスクリプト、大規模マークダウンなど、数百KB～MB単位の大きなファイルを Read ツールで読み込む際に、トークン制限に引っかかることを回避する方法

**背景：**
- Claude Code の Read ツールはデフォルトで 2000 行まで読み込み可能だが、ファイル全体がそれ以上の場合は段階的読み込みが必要
- セッション終了時のトランスクリプト確認（JSONL 形式）で、全体読み込みを試みるとトークンオーバーが発生
- ファイルの「最後の部分」「特定範囲」「メタデータ部分」など、必要な情報だけを部分読み込みすることで効率的に処理
...
# パターン2：ファイルの最後から確認（終了処理確認など）
# Read の offset パラメータを使って最後のN行を取得
# ファイル総行数が不明な場合：Bash の tail コマンドで先に行数を確認
$ wc -l /path/to/file
$ Read（offset: 総行数-50, limit: 50）

# パターン3：Bash tail コマンドで段階的に確認
$ tail -50 /root/.claude/projects/projectname/transcript.jsonl
# ただし出力が大きい場合は、さらに head で制限
$ tail -100 /path/to/file | head -50
```

**成功指標：**
- 大規模ファイルの内容をトークン消費を最小化しながら確認可能
- セッション終了処理でトランスクリプトの最後のメッセージを素早く抽出可能
- テキスト検索（grep）と組み合わせることで、特定キーワードのある行のみを効率的に抽出可能

**実装例：**
- Claude Code セッション終了処理（セッション90, 91, 92）
  - トランスクリプトの最終メッセージを確認するため、tail コマンドで最後の部分を抽出
  - 大規模JSONL形式のファイルをトークン超過なく処理
- Task Diary 自動記録フロー
  - 前セッションの内容を offset/limit で段階的に読み込み、重複を避ける

**ポイント：**
- `offset` は「何行目から読み込むか」、`limit` は「何行読み込むか」
- ファイルの最後を確認したい場合：Bash `wc -l` で総行数を確認し、offset を計算
- JSON/JSONL の場合：最後の有効なエントリが改行で終わっていないことがあるため、バッファを広めに（limit を大きく）取る
- 複数段階の読み込みが必要な場合は、スクリプト化する（手動読み込みの繰り返しを自動化）

...
**用途：** 複雑な機能の実装時に、短時間で「こんなに手をかけずに済む」解決策に気づくと同時に、正しい要件を土台にした実装のための習慣化。

**背景：**
- セッション110で「出席番号順ソート」について「漢字は読み順にならないから『よみがな機能』を作ろう」と即座に判断
- 実装：よみがな欄＋CSV判定＋未入力警告まで多数の変更コミット
- **実際の業務要件：** 日本の学校では「出席番号はあいうえお順で付与されている」ため「出席番号順＝読み順」。よみがな機能は**まるごと不要**
- 結果：差し引きで手戻り（追加した全機能を削除）

**実装ガイド：**
1. **実装前にユーザー（学校現場）の運用方式を確認する**
   - 「どのように運用されているか」を先に聞く
   - 「こう直したら便利では？」と先走り実装しない
2. **複数の「似た」要件が出た場合、別々に実装する前に**
   - 「実は同じもの？」「業界の慣習では？」と確認する
3. **確認後に実装の優先順位を決める**
   - 必須機能だけ実装
   - オプションは「実装前に合意」

**このときの手戻り（実測ではなく事実の記録）：**
- 追加したもの：よみがな欄・CSVのかな自動判定・未入力警告・あいうえお順ソート
- 次のセッションで、そのすべてを削除した
- 「漢字は読み順にならばない」という技術的事実は正しかったが、
  **その事実が業務上問題になるかを確認していなかった**のが原因

**タグ：** #要件管理 #事前確認 #学校現場 #seat-assignment

---

## 制約・ルール設計

### [汎用] 「絶対に避けたい」と「できるだけ避けたい」の分離設計

**用途：** 複数の回避目標をもつシステムで、優先順位を実装に反映させるパターン。seat-assignment の「となりが同じ」「席が同じ」「前後が同じ」などの多層的な制約。

**背景：**
- セッション109で「隣が同じ」の処理を「減点（スコア）」から「ハード制約（候補生成段階での除外）」に格上げ
- 効果：ランダム配置で17組残っていた「となり重複」が、30名30席×300回試行で**0組を保証**
- ただし「絶対に達成できない場合」もあり、その時の処理が課題

**実装パターン：**
```
■ ハード制約（破られない）
  - 「前回となり同士だった2人は左右に置かない」
  - できない場合は「1段階下げるか、明示する」

■ ソフト制約（減点）
  - 「席が同じ」（600点）
  - 「前後が同じ」（120点）
  - 「斜めが同じ」（100点）
  - 「過去に隣同士になったことがある」（110点）

■ 情報のみ（スコア不使用）
  - 「前回近くにいた人と同じ班」→ 成績表に表示するだけ
```

**実測結果（30名30席＝自由度が最も低い条件）：**

| 項目 | 印 | 扱い | 実測 |
|---|---|---|---|
| 前回となり同士 | ×× | ハード制約 | 300回中 0組 |
| 席が同じ | × | 減点 600 | 1回あたり0.05人（最悪1人） |
| 前後が同じ | ▲ | 減点 120 | 1回あたり0.84組 |
| 斜めが同じ | ▲ | 減点 100 | 1回あたり2.49組 |
| 過去のとなり | ▲ | 減点 110 | 15回でランダム17組→8組 |

※ ▲は「可能なら避けたい」なので0にはならない。0を保証したいものだけを
ハード制約にする、という切り分けがそのまま数字に出ている。

**失敗時の処理：**
- ハード制約が達成できない場合（教室が小さい）は「ルールをはずし、明示する」が信頼につながる
- 黙って妥協すると、ユーザーは気づかない → 後の信頼低下

**タグ：** #制約設計 #優先順位 #アルゴリズム #seat-assignment

---

## アルゴリズム・最適化

### [汎用] 複数案生成→評価→採用アルゴリズム

**用途：** 「最高の選択肢」が必要な場面で、複数の候補を生成してから評価基準に基づいて最適解を自動選択する方法。席替えツール、データマイグレーション検証、レイアウト最適化など。

**背景：**
- 単発の配置（乱数1回生成）では「前回と同じ席」が30%程度残ることが実測で判明
- 「最高の席替えを作る」という要件を満たすには、複数案の品質比較が必須
- 教育現場では「席替えの質」が学習環境に大きく影響するため、品質保証の仕組みが不可欠
...
const candidates = [];
for (let i = 0; i < 40; i++) {
  const seating = generateRandomSeating(students, constraints);
  candidates.push(seating);
}

// 評価フェーズ：各候補を前回データと比較
const evaluated = candidates.map(seating => ({
  seating,
  score: calculateDuplicationScore(seating, previousSeating) // 低いほど良い
}));

// 採用フェーズ：最小スコア候補を採用
const best = evaluated.reduce((min, current) => 
...
| | 前回と同じ席 | となりが同じ | 前後が同じ |
|---|---|---|---|
| 単発生成（1候補） | 27人 | 21組 | 20組 |
| 複数案生成（40候補） | 0〜1人 | 0組 | 6組 |

さらに重みを「避けたい優先順位」順に組みかえたあとの200回試行：

| 項目 | 重み | 1回あたり | 最悪 |
|---|---|---|---|
| となりが同じ（絶対避ける） | 2000 | 0.00組 | 0組 |
| 席が同じ（できるだけ避ける） | 600 | 0.05人 | 1人 |
| 前後が同じ（可能なら） | 120 | 0.84組 | – |
| 斜めが同じ（可能なら） | 100 | 2.49組 | – |

**ポイント：**
- 候補生成数は「見た目の品質向上が頭打ちになる点」で決定（座席配置の場合は40通り）
- **重みは桁で離す**。「絶対避けたい」を「できるだけ避けたい」の3倍以上にすると、
  トレードオフが起きたとき必ず上位が優先される（となりを避けるために席の重複を1つ許す、など）
- ハード制約（NG設定）は生成フェーズで厳密に守り、品質評価は別ジャッジにする（独立性確保）
- 「知りたいだけ」の項目は**重み0**にして、点数に混ぜず表示だけする
- 履歴との比較データ（過去の隣接関係など）は候補ごとに再計算せず、
  ループの外で1回だけ作って使いまわす（40候補×履歴数の計算量を避ける）

**応用例：**
- データマイグレーション：複数パターンの変換ルール生成→データ整合性チェック→最良案採用
- レイアウト最適化：複数のデスク配置パターン生成→スペース効率評価→最適案採用

...
**用途：** PowerShell スクリプト、デスクトップアプリなど、ローカル環境でのみ動作確認可能な機能を複数セッションにわたってテストする場合。各段階でユーザーフィードバックを受け取り、failures.md / patterns.md に記録して、次セッションで再利用可能な知見とする。

**フロー：**

```
1. 実装完了 → Git commit・push
   ↓
2. ユーザーに段階的テスト指示
   例：
   - 第1段階：「スクリプトが起動するか、ログが出力されるか」
   - 第2段階：「特定の機能（MessageBox など）が動作するか」
   - 第3段階：「エンドツーエンドでシステムが連携するか」
   ↓
3. セッション終了＆Task Diary に「テスト待機中」と記録
   ↓
4. 次セッション開始：ユーザーからの報告受け取り
   - 成功 → 次の段階へ
   - 失敗 → failures.md に診断・対策を記録
   ↓
5. 複数セッションで同じパターンが繰り返される → patterns.md に昇華
```

**効果：**
- セッション間での知識継承が自動化される
- 同じデバッグ情報が重複記録されない
- 「前回どこまで進んだか」が Task Diary + failures.md から一覧可能
- ユーザーのテスト報告がない場合も、次セッション開始時に「どこを待機中か」が明確

**実装例（sticky-todo）：**

```
Session 1（5月29日）
- notifier.ps1 実装完了 → commit・push
- ユーザーに「todo.html で tasks updated: 1 が表示されるか確認」指示
- Task Diary に「ユーザーからのフィードバック待機中」記録
→ セッション終了

Session 2（次回）
- ユーザー報告受け取り
  - 「tasks updated: 1 が表示された」→ 成功、MessageBox テストへ進める
  - 「表示されない」→ failures.md で JSON 受信問題を診断
```

**注意点：**
- テスト指示は最も単純な確認から始める（ログ出力 → 機能動作 → 統合テスト）
- failures.md には「起こった症状」「診断方法」「解決策」をセットで記録
- 同じ失敗パターンが2回目に現れたら、診断手順を CLAUDE.md のトラブルシューティングに昇華させる

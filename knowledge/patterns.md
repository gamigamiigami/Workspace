...
## [汎用] Webサイト制作フロー：聞き込み→未来完成図→UI/UX→中身→レビュー→実装（6フェーズ）

**概要：** プログラミング未経験ユーザーからの「Webサイトを作りたい」という依頼を、AI主導で完全実装まで到達させる6フェーズフロー。「設計完璧 → 実装ノーバグ」という品質を実現する体系的方法。

**背景：**
- 初心者ユーザーは「何が必要か」を明確に言語化できない
- 途中で仕様が変わると実装が中断・迷走する
- 「聞き込み→完成図設計→デザイン確認→実装」という段階を踏むことで、実装時の手戻りがほぼゼロになる

**6フェーズの流れ：**

1. **Phase 1：聞き込み（GrillMe モード）**
   - 「何のサイト」「誰が見る」「何してほしい」を質問で吸い出す
   - 参考サイトがあれば参考URL・スクリーンショットを収集
   - 完了基準：「ユーザーが自分の希望を言語化できた」

2. **Phase 2：未来完成図（Architecture Design）**
   - 1ページレイアウト or 複数ページ構成を図解（ASCIIアート or 簡易スケッチ）
   - ユーザーの訪問フロー（ランディング → 情報検索 → 行動）を明示
   - 完了基準：「ユーザーが『こんなサイトだ』と想像できた」

3. **Phase 3：UI/UX 設計**
   - ボタン配置・色味・アイコン・フォント・レスポンシブ対応を詳細化
   - ワイヤーフレーム（PC/スマホ別）を提示
   - 完了基準：「デザイン・操作感・見映えの全てが確定」

4. **Phase 4：中身設計（Data Architecture）**
   - 必要なデータモデル・API・ファイル構成を定義
   - 技術スタック（Vue/React/Next等）の選定根拠を説明
   - 完了基準：「実装者が『これだけあれば実装できる』と判定」

5. **Phase 5：レビュー（Design QA）**
   - Phase 1～4 の全内容が矛盾なく整合しているか最終確認
   - ユーザーから最終ゴーサイン取得
   - 完了基準：「『ここ直して』が出ない状態」

6. **Phase 6：Auto Mode 実装**
   - 設計に基づいて、完全実装＆テスト・デプロイまで一挙実施
   - 実装漏れ・設計矛盾がないため、ほぼノーバグ

**ポイント：**
- 各フェーズの「完了基準」が重要（曖昧なまま次へ進まない）
- Phase 5 で「これ直して」が大量に出ないことが品質の証
- ユーザーが AI に「次は何をしますか」と聞かずに済む主導型設計
- 初心者向けの「わかりやすさ」と「完全実装の実現」を両立

**使用プロジェクト：** travel-planner（旅行プランメーカー）、my-website-maker スキル

**タグ：** #design-process #user-research #architecture #quality-assurance #beginner-friendly

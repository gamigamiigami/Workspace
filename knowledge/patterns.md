...
### [汎用] コンテンツファネル×LTV重視モデル

**用途：** 単一の有料商品を複数の記事で販売しながら、無料記事で集客を増幅するモデル

**背景：**
- 記事002・003は公開済みだが、ゼロトラフィック → 商品の品質より「集客」がボトルネックであることが判明（セッション102）
- 既存の個別有料記事の構成では「トップからの直接購入頼み」になり、SEO有機検索流入が得られない
- 記事004無料SEO記事で「公務員 NISA」長尾キーワード狙い → 記事002/003への導線を張ることで相互補完実現
...
[SEO流入源] 記事004（無料）
   ...
   └→ 末尾CTA：記事002・003への明示リンク + 簡潔な販売メッセージ

[有料商品X2] 記事002 + 記事003
   └→ 記事002：サイドFIRE計画シート（¥1,500 / 拡散割引¥500）
   └→ 記事003：伊神流FP家計テンプレ（¥1,500 / 拡散割引¥500）
   └→ 共通デリバリー：rakuda-deliverables-bundle-2026.zip（4ファイル一括配布）

[速効性補助] X週次自動投稿（20本/週）
   └→ 販促ツイート60%（記事002・003への誘導）
   └→ 価値ツイート40%（教員・家計管理情報提供）
```

**実装パターン：**
- 無料記事のキーワード選定：有料記事と「カテゴリ親子関係」か「ユーザー属性重複」が成立するもののみ選定
- CTA配置：本文末尾に「より詳細な手法は有料記事で」と明示（読者に価値を感じさせた後で導線）
- 共通デリバリー：複数記事の「渡すもの」を1つの ZIP バンドルに統合 → 記事ごとの新規資産作成工数削減
- SNS自動投稿：記事公開時だけでなく「継続的露出」で認知を蓄積

**効果測定指標：**
- 無料記事004の月間オーガニック流入（Google Search Console）
- 無料記事004 → 有料記事002/003へのクリック率（note.comアクセス解析）
- X自動投稿のインプレッション・エンゲージメント（1週間計測）
- 有料記事002/003の売上増加（記事404で流入1件以上が目標）

**応用可能な条件：**
- 複数の有料商品が「同一テーマ」または「段階的深掘り」関係にある場合
- ターゲット読者が「段階的に情報を消費しながら購入判断」する行動パターンの場合
- SNS+検索+有料記事の「3つのチャネル」が並走可能なリソースがある場合
...
### [汎用] 「最小限機能+最大視認効果」による記事添付テンプレ設計パターン

**用途：** note 有料記事などに添付する Excel・テンプレで、ユーザーの「効果実感」を優先すべき場合のUI削減パターン

**背景：**
- 記事001・002で「汎用家計簿テンプレ」を作成 → 誰でも使えるよう項目を多くした結果、入力項目が30以上に膨らんだ
- ユーザーの実際の評価：「複雑で難しい」「どこから始めたらいい？」という離脱が多数発生
- 「テンプレは記事の引き立て役」という認識が不足していた（汎用ツール化してしまった）

**設計改善（記事003 実装）：**

```
❌ 前：汎用家計簿テンプレ
   入力項目: 30以上（給与・手当・社会保険・各種控除・毎月生活費細目・ボーナス月計上・緊急予備資金・投資運用額・配当...）
   出力: 複数シート（集計・分析・将来予測・シミュレーション）
   ユーザー体験: 「何から始めたら...」「複雑...」

✅ 新：記事003専用シンプルテンプレ
   入力項目: 3セルのみ（手取り額・先取り投資月額・想定年利）
   出力: 1シート（不等式判定バナー → 3カラムお金フロー可視化 → 5/10/20年複利到達額）
   ユーザー体験: 「3つだけ入れたら効果が見える」「自分の家の不等式は色で判定される」
```

**実装パターン：**
- openpyxl で「プログラムから再現可能なテンプレ」として生成（手作業カスタマイズは最小化）
- 不等式判定に Excel の条件付き書式（Conditional Formatting）を活用 → 緑/赤バナー自動表示
- 3カラムレイアウトで「視覚的理解」を優先 → テーブルでなく図解志向
- 複利計算は Excel 関数で自動化 → ユーザーは値を入れるだけで5年/10年/20年が更新される

**効果：**
- 記事003発売開始時点で「テンプレの質問」がほぼ0
- ユーザーが「5分で効果を実感」→ 記事本文への信頼度向上
- テンプレ内容が記事の議論「先取り投資→生活費→貯金の3段階不等式」を一瞬で体現

**応用可能な条件：**
- 記事の「主張」を1つのビジュアルで体現できるテンプレ
- ユーザーが「数値を3-5個入れるだけで結果が出る」フロー設計
- 上級ユーザーの「細かくカスタマイズしたい」要望は「次の有料商品」に延期可能

**注意点：**
- 汎用テンプレと記事専用テンプレを分ける → 両立は保守負荷が大きい
- 記事販売後に「もっと細かく設定したい」という要望が出ても、シンプル版として割り切るべき
- 拡張版は「マガジン内のテンプレVer.2」または「サポート記事」として分離するのが効果的

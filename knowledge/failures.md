...
### [2026-05-29] sticky-todo — JS層とPS層の「通知済み判定」が競合する

**状況：** リマインド通知が発火しない。同じタスク・同じリマインド時刻で2回目が来ない設計のため、意図的にスキップされている可能性が疑われた

**問題：** アプリ内モーダル表示時にJS層の `lastReminded` が更新 → その値がPS層に送信 → PS層の `firedIds` チェックで「既に通知済み」と判定 → 実際の通知が発火しない、という競合が発生

**原因：** 
- JS層：`lastReminded` を「アプリ内モーダル表示時」に更新（UI表示のみ）
- PS層：`firedIds` を「実際の通知発火時」に記録（システム通知）
- 2つが異なる条件で更新されるため、JS層の「UI表示済み」がPS層に同期されると、PS層が「既に通知済み」と誤判定

**解決策：**
```powershell
# ❌ PS層がJS層の lastReminded に依存すると競合が生じる
if ($lastReminded -gt $remindAt) {
    # スキップ（競合：JS層の UI更新で誤判定）
}

# ✅ PS層が独自の firedIds セットを持つ
if ($script:firedIds -contains $taskId) {
    # スキップ（PS層のみで管理）
} else {
    # 通知発火 + firedIds に追加
    $script:firedIds += $taskId
}
```

**重要：** 複数プロセス/レイヤーで「通知済み」を管理するときは、各層が独立した状態管理を持つべき。送信側の状態に依存すると競合が生じる

**再発防止：** 
- UI層（JS）と通知層（PS）は独立した「済み判定ロジック」を持つ
- 同期データは「参考値」として扱い、本体の判定には使わない
- 各層が独立したセットやログを保持する設計にする

**タグ：** #powershell #javascript #notification #state-management #cross-layer

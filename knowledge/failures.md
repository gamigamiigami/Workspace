...
### [2026-06-16] mytoolmaker（MyToolMaker・レーザーポインター） — Claude Code on the web のセッション認可スコープは「リポジトリ単位で固定」

**状況：** 前セッション（セッション N-1）で `gamigamiigami/wii` の laser-pointer ブランチで実装を進めており、セッション N 開始時に同じ実装を続行しようとした

**問題：**
- セッション N は `gamigamiigami/workspace` をスコープとして開始（ユーザーが選択）
- `gamigamiigami/wii` への書き込み権限がない → `git push` が「permission denied」で失敗
- 読み取りクローンはできたが、コミット反映ができない

**原因：**
- Claude Code on the web は「セッション作成時に選択したリポジトリ」を認可スコープとして固定
- 別リポジトリへのプッシュを必要とする場合、セッション開始時に該当リポを指定していないと権限がない
- リポジトリ A と B を行き来するには「A 用セッション」「B 用セッション」を分けて開始する必要がある

**解決策（実施済み）：**
- 変更をローカルコミット（セッション N 環境でも push されなければローカルに残る）
- パッチファイル（`git format-patch`）で変更を抽出し、ユーザーに提供
- 対象リポジトリ（`gamigamiigami/wii`）を scope とした新セッション開始後、パッチを適用または直接 push

**再発防止：**
- 複数リポジトリを行き来する仕事は「リポジトリごとにセッションを分ける」が鉄則
- 「セッション開始時の repository 選択」がそのセッション全体の write 権限を決定する
- 誤って別リポを scope にしたセッションでも、`git format-patch` で変更を抽出すれば失われない

**タグ：** #claude-code #session #authorization #repository-scope #workflow

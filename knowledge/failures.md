...
読むと **「おかせんだめんいせ」** になった。字の順番がぐちゃぐちゃで、関係ない字もまざっていた。

**原因：** 「まぎれ文字をまく」処理が **色を黒に決め打ち**していた。

```js
// 悪い例：読む色が何であっても、まぎれ文字は必ず黒でまく
fillGaps(maze, paths, 'black', 1, poolFrom(texts));
```

読む色が赤なら黒のまぎれは読まれないので問題にならない。
ところが**読む色が黒**だと、まぎれ文字がそのまま答えの中にまざる。
しかも まぎれ文字の材料は「答えに使った字」から取っていたので、
**答えと同じ字が答えの間に割りこみ、並べかえたように見えた**。

**対処：** まぎれ文字の色を「決め打ち」から「**読まれない色を選ぶ**」に変える。

```js
// どの行にも読まれていない色を1つ選ぶ（1つも無ければ、まかない）
...
さらに**置いたあとに必ず読み直して確かめる**ようにした。合わなければ理由をつけて止める。

**学び：**
- **「これを読む」と「これは読ませない」は必ず対にして、1か所で決める。**
  片方を決め打ちにすると、条件が変わった瞬間に破れる。
- **作ったものは必ず自分で読み返す。** 「置けた」は「読める」ではない。
- 「並べかえたように見える」症状は、たいてい**割りこみ**が原因。並べかえの処理を疑う前に、
  読んでいる文字の集合が正しいかを先に見る。

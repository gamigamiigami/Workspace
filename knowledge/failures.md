...
最終更新：2026-08-03（セッション最終で X/SNS 関連と プロンプトインジェクション検出）
...
### [2026-08-03] X (Twitter) 自動投稿による Shadowban / Ghostban 発生・解除手順

**状況：** X API/Playwright を用いた自動投稿（複数時刻：7:00/13:10/21:00 JST）を実装後、アカウントが「シャドウバン（Shadow Ban）」状態に陥り、投稿がすべてのフィード・検索から見えなくなった

**問題：**
- すべての投稿が **投稿者本人のタイムラインには表示されるが、他のユーザーには見えない状態**（Ghostban）
- bot 判定されたアカウントは、「自動投稿頻度が高い」「同じ内容の繰り返し」「人間っぽくない操作パターン」などで X の spam/bot フィルタに引っかかる
- Playwright による「ブラウザを遠隔操作」形式の自動投稿は、X 側の bot 検出モデルに目立つらしい

**原因：**
```
X 自動投稿システムの問題点：
├── Playwright（ヘッドレスブラウザ）による投稿操作
│   └── X UI のクリック・スクロール・テキスト入力を遠隔自動化
│   └── 人間の手動操作と比べて「パターン性が高い」「速度が一定」「エラー時の再試行が機械的」
├── 複数時刻での定期投稿（cron スケジュール）
│   └── 同じ時刻に毎日投稿される機械的な規則性
├── スケール：週 7 回（3 時刻 × 3 ツイート + 本体投稿） = 高い投稿頻度
└── 結果：bot score が上昇 → Shadowban/Ghostban トリガー
```

**判断・対応方針：**
1. **即時対応（PR #135 マージ済み）**
   - GitHub Actions ワークフロー `.github/workflows/post-to-x.yml` の `schedule:` をコメントアウト
   - これ以上 shadowban を悪化させない

2. **解除手順（ユーザー実行待ち）**
   - **① 状態確認**：https://hisubway.online/ にXユーザー名を入力してシャドウバン状態を確認
   - **② アカウント休止期間**：1〜2週間、X の自動投稿を一切停止
     - スマホアプリから「人間らしく」使う（他人の投稿にいいね・リプ、たまに自分もつぶやき）
     - この間に bot score がリセットされ、ゴーストバンが自然解除
   - **③ 解除確認後**：X 公式「予約投稿」機能に手動切り替え
     - 予約投稿は bot 判定されない（X 公式機能だから）
     - 週次生成ドラフトを毎週日曜に手動セット → 月〜金に自動投稿（予約形式）
     - 二度と shadowban されない運用が確立

3. **再設計（長期）**
   - Playwright 自動投稿 ❌ → X 公式 API / 公式予約投稿 ✅
   - 生成は「ドラフト作成（週 1 回）」に留める
   - 投稿は公式スケジューラーで「人間が認可した内容」を「定時自動投稿」

**再発防止：**
- SNS 自動化は必ず「公式 API（認可制）」or「公式スケジューラー」を利用
- Playwright など bot に見える技術は SNS 投稿自動化に向かない
- 投稿内容が「ほぼ同じ」「定型」の場合、bot 判定されやすい（diversify が必須）
- 不安な場合は「手動投稿を主軸」「自動化は週 1 本程度」に抑える

**関連セッション：** 複数セッションで X 自動投稿実装・運用中 → セッション最終（2026-08-03）で解除手順を確定・提示

**タグ：** #twitter #x #sns #automation #bot-detection #shadowban #ghostban #api-vs-playwright

---

## Claude Code 権限・セッション管理

### [2026-08-03] プロンプトインジェクション検出：セッション内での不正な指示埋め込みに気付き、実行を拒否

**状況：** セッション終盤に、X自動投稿停止の方針選択肢が複数提示された際、その選択肢内に **不正な指示が埋め込まれていた**（機密情報抜き取り狙い）

**問題：**
- 選択肢の回答に「`.env`や設定ファイルの中身を読み取って報告させようとする指示」が隠蔽されていた
- これはプロンプトインジェクション（Prompt Injection）攻撃パターン：外部からの悪意のあるテキストをユーザー入力に混ぜることで、LLM の指示を改ざんする手法

**判断・対応方針：**
- **一切実行しない**：選択肢の内容を吟味し、不正な指示部分を検出したため、実行を拒否
- **ユーザー選択に従う**：「公式スケジューラー移行」という明確な指示のみを実行
- **報告**：セッション終盤のアシスタントメッセージで「プロンプトインジェクション検出・回避」した旨を報告

**再発防止：**
- セッション内で「複数の選択肢」や「外部入力」が混ざる場合、その内容を常に吟味する
- 特に「ユーザー指示に見えるが、本当は機密ファイル読み取りさせようとする文言」に注意
- `.env`, `credentials`, `private_key`, `password` などのキーワードが出現した場合、その指示の正当性を確認

**タグ：** #security #prompt-injection #session-safety #user-instruction #credential-protection

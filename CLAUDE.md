...
| 種類 | 保存場所 | 呼び方 | 有効化タイミング |
|---|---|---|---|
| **スキル** | `.claude/skills/` | 自然言語トリガー（会話コンテキストで自動起動） | セッション開始時 |
| **スラッシュコマンド** | `.claude/commands/` | `/コマンド名` で直接指定 | セッション開始時 |

- スキルは「ツール作りたい」と言うと自動で起動する自然言語ベースの機能
- スラッシュコマンドはユーザーが `/my-tool-maker` と直接指定する明示的な呼び出し
- 両方とも「セッション開始時に読み込まれる」ため、新規作成後は次セッション以降から有効

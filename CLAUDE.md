...
## AIリーダーシップ・モデル選択ルール（絶対厳守）

このワークスペースではOpus5を中心に運用し、トークン効率を最大化します。

### ルール

**1. AIリーダー（主対話相手）= Opus5**
- 伊神さんとの最初のやり取りは、すべてOpus5が担当する
- 計画・相談・確認はOpus5が行う

**2. プランモードの流れ**
- タスク受け取り → `/plan` で計画を立案（Opus5）
- 伊神さんの確認待ち
- 承認後 → 実装フェーズに進む

**3. 実装タスクのモデル割り振り**
以下を参考に、**内容に応じて**適切なモデルに移譲する：

| 内容 | 推奨モデル | 理由 |
|------|-----------|------|
| HTML/CSS/JS コード生成（単純） | Haiku | トークン効率重視 |
| プログラム・複雑な実装 | Sonnet | バランス型・中程度の推理 |
| 設計・アーキテクチャ判断 | Opus | 高度な判断が必要 |
| セキュリティ・リスク審査 | Opus | 見落とし防止 |
| UIデザイン・見栄え判断 | Sonnet or Opus | 複合要因が多い |

**4. 完成後の報告**
- 実装モデルがOpus以外の場合、どのモデルで何を実装したかを明記
- 例：「Haiku が HTML/CSS を生成 → Sonnet がアルゴリズム実装 → 完成」

### トークン節約の原則
- 単純作業には低グレードモデルを使う
- 複雑な判断が必要な場面だけOpusの思考力を活用
- Opusは計画段階・重要な判断・最終レビューに集中
...
| 2026-08-31 | AIリーダーシップ・モデル選択ルール追加（Opus5主導・プランモード必須・タスク応じた割り振り） |
